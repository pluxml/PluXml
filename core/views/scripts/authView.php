		...
		<section class="item-center w350p">
			<div class="logo"></div>
				<div class="auth">
                		...
                		<form action="auth/login<?= !empty($redirect)?'?p='.$plxUtils->strCheck(urlencode($redirect)):'' ?>" method="post" id="form_auth">
                			<fieldset>
                				<?= $plxToken->getTokenPostMethod() ?>
                				<h1 class="h5-like txtcenter"><?= L_LOGIN_PAGE ?></h1>
                				<?php (!empty($msg))?$plxUtils->showMsg($msg, $css):''; ?>
               					...
               					<label class="w100">
               						<?= L_AUTH_PASSWORD_FIELD ?>
               						<?php $plxUtils->printInput('password', '', 'password','',false, 'w100');?>
               					</label>
                				<?php 
                				if ($plxAdmin->aConf['lostpassword']) {
                				?>
               						<p><a href="?action=lostpassword"><?= L_LOST_PASSWORD ?></a></p>
                    			<?php 
                				}
                                eval($plxAdmin->plxPlugins->callHook('AdminAuth'))
                                ?>
               					<div class="txtcenter">
               						<input role="button" class="btn--primary" type="submit" value="<?= L_SUBMIT_BUTTON ?>" />
                				</div>
                			</fieldset>
                		</form>
                	</div>
               		<p class="mas">←&nbsp;<a href="<?= $plxUtils->getRacine() ?>"><?= L_BACK_TO_SITE ?></a></p>
...
<!-- 
            ...
            ?>
    				<div class="auth">
                		<?php eval($plxAdmin->plxPlugins->callHook('AdminAuthTop')) ?>
                		<form action="admin/lostPassword<?= !empty($redirect)?'?p='.$plxUtils->strCheck(urlencode($redirect)):'' ?>" method="post" id="form_auth">
                			<fieldset>
                				<?= $plxToken->getTokenPostMethod() ?>
                				<h1 class="h5-like txtcenter"><?= L_LOST_PASSWORD ?></h1>
               					<label class="w100">
               						<?= L_AUTH_LOST_FIELD ?>
             					...
             					<p><a href="?p=/core/admin"><?= L_LOST_PASSWORD_LOGIN ?></a></p>
            ...
            ?>
    					<div class="auth">
                    		<?php eval($plxAdmin->plxPlugins->callHook('AdminAuthTop')) ?>
                    		<form action="admin/changePassword<?= !empty($redirect)?'?p='.$plxUtils->strCheck(urlencode($redirect)):'' ?>" method="post" id="form_auth">
                    			<fieldset>
                    				<?= $plxToken->getTokenPostMethod() ?>
                    				<input name="lostPasswordToken" value="<?= $lostPasswordToken ?>" type="hidden" />
                    				<h1 class="h5-like txtcenter"><?= L_PROFIL_CHANGE_PASSWORD ?></h1>
                   					<label class="w100">
                   						<?= L_PROFIL_PASSWORD ?>
                   						<?php $plxUtils->printInput('password1', '', 'password', '', false, 'w100', '', 'onkeyup="pwdStrength(this.id)"') ?>
                   					</label>
                   					<label class="w100">
                   						<?= L_PROFIL_CONFIRM_PASSWORD ?>
                   						<?php $plxUtils->printInput('password2', '', 'password', '', false, 'w100') ?>
                   					</label>
               						<p><a href="?p=/core/admin"><?= L_LOST_PASSWORD_LOGIN ?></a></p>
                    				<?php eval($plxAdmin->plxPlugins->callHook('AdminAuth')) ?>
                    				<div class="txtcenter">
   										<input role="button" class="btn--primary" type="submit" name="editpassword" value="<?= L_PROFIL_UPDATE_PASSWORD ?>" />
                    				</div>
                    			</fieldset>
                    		</form>
                    	</div>
            ...
            ?>
                        ...
                        <?php eval($plxAdmin->plxPlugins->callHook('AdminAuthTop')) ?>
							...
							<p class="mts"><a href="?p=/core/admin"><?= L_LOST_PASSWORD_LOGIN ?></a></p>
							<?php eval($plxAdmin->plxPlugins->callHook('AdminAuth')) ?>
                    	</div>
                   		<p class="mas">←&nbsp;<a href="<?= $plxUtils->getRacine() ?>"><?= L_BACK_TO_SITE ?></a></p>
			<?php
