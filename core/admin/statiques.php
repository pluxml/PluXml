                        ...
                        foreach ($plxAdmin->aStats as $k => $v) { # Pour chaque page statique
                            ...
                            <tr>
                                <td>
                                    ...
                                    <input type="hidden" name="<?= $k ?>_template"
                                           value="<?= plxUtils::strCheck($v['template']) ?>"/>
                                <td><?= $k ?></td>
                                <td>
                                    <input title="<?= L_STATICS_PAGE_HOME ?>" type="checkbox" name="homeStatic[]"
                                           value="<?= $k ?>"<?= $selected ?> />
                                </td>
                                ...
                                <td>
                                    ...
                                    if (!PlxUtils::checkSite($url)) {
                                        ?>
                                        <button><a href="statique.php?p=<?= $k ?>" title="<?= L_STATICS_SRC_TITLE ?>"><i
                                                        class="icon-pencil"></i></a></button>
                                        <?php
                                        if ($v['active']) {
                                            ?>
                                            <button>
                                                <a href="<?= $plxAdmin->urlRewrite('?static' . intval($k) . '/' . $v['url']) ?>"
                                                   title="<?= L_STATIC_VIEW_PAGE ?> <?= PlxUtils::strCheck($v['name']) ?> <?= L_STATIC_ON_SITE ?>"><i
                                                            class="icon-eye"></i></a></button>
                                            <?php
                                        }
                                    } elseif ($v['url'][0] == '?') {
                                        ?>
                                        <button><a href="<?= $plxAdmin->urlRewrite($v['url']) ?>"
                                                   title="<?= PlxUtils::strCheck($v['name']) ?>"><i
                                                        class="icon-eye"></i></a></button>
                                        <?php
                                    } else {
                                        ?>
