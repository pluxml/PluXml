	...
	<table id="articles-table" class="full-width">
		...
		if($arts) { # On a des articles
			...
			while($plxAdmin->plxRecord_arts->loop()) { # Pour chaque article
				...
				echo '<td>'.$libCats.'&nbsp;</td>';
				echo '<td style="white-space:nowrap"><a title="'.L_NEW_COMMENTS_TITLE.'" href="comments.php?sel=offline&amp;a='.$plxAdmin->plxRecord_arts->f('numero').'&amp;page=1">'.$nbComsToValidate.'</a> / <a title="'.L_VALIDATED_COMMENTS_TITLE.'" href="comments.php?sel=online&amp;a='.$plxAdmin->plxRecord_arts->f('numero').'&amp;page=1">'.$nbComsValidated.'</a>&nbsp;</td>';
