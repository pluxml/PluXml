...
<form action="user.php" method="post" id="form_user">
	...
	<fieldset>
		...
		<div class="grid">
			...
			<div class="col sml-12 med-7">
				<?php plxUtils::printInput('email', plxUtils::strCheck($plxAdmin->aUsers[$id]['email']), 'email', '30-255', false, '', '', '', $requireMail); ?>
			</div>
		</div>
		<div class="grid">
			<div class="col sml-12">
				<label for="id_content"><?= L_USER_INFOS ?>&nbsp;:</label>
				<?php plxUtils::printArea('content', plxUtils::strCheck($plxAdmin->aUsers[$id]['infos']), 0, 8, false, 'full-width', 'placeholder=" "'); ?>
			</div>
		</div>
	</fieldset>
