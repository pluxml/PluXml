<?php


# install.php
const L_INSTALL_TITLE					= 'Installacion';
const L_WRONG_PHP_VERSION				= 'PluXml requesís PHP 5 o superior per foncionar.';
const L_SELECT_LANG						= 'Seleccionatz vòstra lenga';
const L_INPUT_CHANGE					= 'Cambiar';
const L_INSTALL_DATA					= 'Installar las donadas d\'exemple';
const L_YES 							= 'Òc';
const L_NO 								= 'Non';
const L_DEFAULT_CATEGORY_TITLE			= 'Rubrica 1';
const L_DEFAULT_CATEGORY_URL			= 'rubrica-1';
const L_DEFAULT_STATIC_TITLE			= 'Estatic 1';
const L_DEFAULT_STATIC_URL				= 'estatic-1';
const L_DEFAULT_ARTICLE_TITLE			= 'Primièr article';
const L_DEFAULT_ARTICLE_URL				= 'primièr-article';
const L_DEFAULT_COMMENT_CONTENT			= 'Aquò es un primièr comentari !';
const L_ERR_PLUXML_ALREADY_INSTALLED	= 'PluXml es ja configurat !';
const L_ERR_MISSING_USER				= 'Volgatz entre-senhar lo nom del redactor !';
const L_ERR_MISSING_LOGIN				= 'Volgatz entre-senhar lo login de connexion !';
const L_ERR_MISSING_PASSWORD			= 'Volgatz entre-senhar un senhal !';
const L_ERR_MISSING_EMAIL               = 'Volgatz picar una adreça e-mail (recuperacion del senhal).';
const L_ERR_PASSWORD_CONFIRMATION		= 'Confirmacion del senhal incorrècta !';
const L_PLUXML_INSTALLATION				= 'Installacion de PluXml';
const L_SITE_DESCRIPTION				= 'Blòg o Cms a l\'Xml !';
const L_VERSION							= 'version';
const L_USERNAME						= 'Nom de l\'administrator';
const L_LOGIN							= 'Identificant de connexion a l\'administracion';
const L_PASSWORD						= 'Senhal';
const L_PASSWORD_CONFIRMATION			= 'Confirmacion del senhal';
const L_EMAIL						    = 'Adreça electronica';
const L_INPUT_INSTALL					= 'Installar';
const L_TIMEZONE						= 'Fus orari';
const L_PWD_VERY_WEAK					= 'Senhal fòrça feble';
const L_PWD_WEAK						= 'Senhal feble';
const L_PWD_GOOD						= 'Senhal bon';
const L_PWD_STRONG						= 'Senhal fòrt';

